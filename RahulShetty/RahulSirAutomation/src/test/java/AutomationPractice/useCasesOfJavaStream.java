...
public class useCasesOfJavaStream {
	...
	public void webTableAutomation() throws Exception {
		...
		List<Object> origList = originalWebElementList.stream().map(s -> s.getText()).collect(Collectors.toList());
		List<Object> sortedList = origList.stream().sorted().collect(Collectors.toList());
